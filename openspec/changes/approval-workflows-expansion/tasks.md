...
- [x] 2.1 RED `tests/Entity/InvoiceTest.php` — `submitForPaymentApproval()` freezes levels(decision 5); `clearPaymentLevel()` order enforcement; `rejectPaymentApproval()` discards progress; amount change post-submission does not reopen cleared levels.
- [x] 2.2 GREEN `src/Entity/Invoice.php` — add `paymentApprovalStatus`/`paymentRequiredLevels`/`paymentCurrentLevel` (D4) + `isPaymentApproved()`/`nextPendingPaymentLevel()`/`submitForPaymentApproval()`/`clearPaymentLevel()`/`rejectPaymentApproval()` per design's Interfaces/Contracts block.
- [x] 2.3 `migrations/Version20260814110000.php` — CREATE `gppro_invoice_payment_approvals` (audit, mirrors `gppro_expense_approvals`) + ALTER `gppro_invoices` add nullable `payment_approval_status`, nullable `payment_required_levels`, `payment_current_level` default 0.
- [x] 2.4 `src/Entity/InvoicePaymentApproval.php` + `src/Repository/InvoicePaymentApprovalRepository.php` — mirror `ExpenseApproval`/`ExpenseApprovalRepository`. Also registered `InvoicePaymentApprovalRepository` as a DI service in `config/services.yaml` (same pattern as PR1's `InvoicePaymentApprovalLevelRepository`).
- [x] 2.5 RED `tests/Invoice/InvoicePaymentApprovalPolicyTest.php` — eligible role/named-approver clears; ineligible denied; already-cleared level not re-approvable. Mirrors `ExpenseApprovalPolicyTest`.
- [x] 2.6 GREEN `src/Invoice/InvoicePaymentApprovalPolicy.php` — mirrors `ExpenseApprovalPolicy` (`canApprove`/`canReject`), including creator-exclusion (uses `Invoice::getUser()`, the closest equivalent to `Expense::getCreatedBy()`), four-eyes, and super-admin bypass.
- [x] 2.7 RED `tests/Invoice/InvoicePaymentApprovalServiceTest.php` — `submit()` resolves+freezes via resolver; `approve()`/`reject()` persist audit rows.
- [x] 2.8 GREEN `src/Invoice/InvoicePaymentApprovalService.php` — mirrors `ExpenseApprovalService`, adds `submit(Invoice)`.
- [x] 2.9 RED `tests/Controller/InvoiceControllerTest.php` — new routes: `submitPaymentApprovalAction`/`approvePaymentAction`/`rejectPaymentAction` reachable and correctly gated.
- [x] 2.10 GREEN `src/Controller/InvoiceController.php` — add the 3 new actions + routes `invoice_{submit_payment_approval,approve_payment,reject_payment}`. Gated by the existing `edit_invoice` attribute (mirrors the design's explicit reuse for the submit action); `approve_invoice_payment`/`reject_invoice_payment` voter attributes are wired in PR3 per tasks.md's own phase split — eligibility for the pending level is fully enforced via `InvoicePaymentApprovalPolicy` inside the service call in the meantime. Also added minimal submit/approve/reject buttons to `templates/invoice/invoice_edit.html.twig` (not itemized in design's File Changes table) so the new routes are reachable with real, session-valid CSRF tokens — a token minted via the test harness's `getCsrfToken()` helper does not validate against the real request/session (confirmed this pre-existing gap also affects `admin_invoice_status`'s existing GET+CSRF pattern, not just these new actions).
- [x] 2.11 RED `tests/Controller/InvoiceControllerTest.php` — dual PAID-gate cases (D6): unsubmitted invoice via `changeStatusAction` denied; unsubmitted invoice via `editAction`/`InvoiceEditForm` status dropdown denied; both entry points explicitly.
- [x] 2.12 GREEN `src/Controller/InvoiceController.php::changeStatusAction()` — inside `if ($status === Invoice::STATUS_PAID)`, before any setter, check `!$invoice->isPaid() && !$invoice->isPaymentApproved()` -> flash error, redirect, skip mutation (D6 point 1).
- [x] 2.13 GREEN `src/Controller/InvoiceController.php::editAction()` — capture `$wasPaid` before `handleRequest()`; after valid submit, if `$invoice->isPaid() && !$wasPaid && !$invoice->isPaymentApproved()` -> flash error, re-render, skip `saveInvoice()` (D6 point 2, closes the `InvoiceEditForm` dropdown bypass).
- [x] 2.14 RED same file — already-PAID invoice comment-only re-save (grandfathered, decision 8) is NOT blocked by either gate.
- [x] 2.15 Run `phpunit tests/Entity/InvoiceTest.php tests/Invoice/InvoicePaymentApprovalPolicyTest.php tests/Invoice/InvoicePaymentApprovalServiceTest.php tests/Controller/InvoiceControllerTest.php` (41/41 pass), `phpstan analyse -c tests/phpstan.neon --no-progress` (1 pre-existing unrelated error in `QuotationControllerTest.php`, 0 new), `lint:twig`, `lint:xliff` (both OK). Full existing Invoice safety-net sweep (`tests/Controller/InvoiceControllerTest.php` 28/28, `tests/Entity/InvoiceTest.php`, `tests/Invoice/*` 324 tests, 3 pre-existing failures in `InvoiceModelDefaultHydratorTest`/`DebugRendererTest` confirmed unrelated and present on `main` too — an unrelated "adjustments" template-variable drift, not caused by this change) — zero regression to PENDING/CANCELED transitions or invoice generation/rendering. Implemented on branch `approval-workflows-expansion-pr2-invoice-state-machine` (base: PR1 branch tip `d669b6a`) — ready for PR to be opened by orchestrator.
...
- [x] T.1 RED `tests/Entity/TimesheetTest.php` — `isApproved()`/`approve(User)` state transitions, mirrors `ExpenseTest`.
- [x] T.2 GREEN `src/Entity/Timesheet.php` — add `approvedBy` (?User, `SET NULL`), `approvedAt` (?DateTimeImmutable), `isApproved()`, `approve(User)` per design's Interfaces/Contracts block.
- [x] T.3 `migrations/Version20260814080000.php` — ALTER `gppro_timesheet` add nullable `approved_by_id` (FK, `SET NULL`), nullable `approved_at`.
- [x] T.4 RED `tests/Voter/TimesheetVoterTest.php` — team lead of project grants `approve`; non-lead denied; self-approval by lead granted (decision 4, no creator-exclusion).
- [x] T.5 GREEN `src/Voter/TimesheetVoter.php` — add `approve`/`reject` attributes using `User::isTeamleadOf(Team $team)` (D2) looping `$timesheet->getProject()?->getTeams()`. Do NOT call the private `RolePermissionManager::checkTeamLeadAccess()` and do NOT reuse `checkTeamAccessTimesheet()` (short-circuits true for owner, breaks self-approval-by-lead-only semantics per D2 rationale).
- [x] T.6 RED same file — approved entry: edit/delete denied for owner AND lead (D7 `isAllowedApproved()` guard).
- [x] T.7 GREEN `src/Voter/TimesheetVoter.php` — add private `isAllowedApproved()` check (same shape as `isAllowedExported()`/`isAllowedInLockdown()`) to `canEdit()`/`canDelete()`, returns `false` when `isApproved()` true.
- [x] T.8 RED `tests/Controller/TimesheetTeamControllerTest.php` — `approveAction` sets approvedBy/approvedAt; `rejectAction` is no-op/flash only, entry stays unapproved/editable (D3, no persisted reject state).
- [x] T.9 GREEN `src/Controller/TimesheetTeamController.php` — add `approveAction`/`rejectAction`, routes `admin_timesheet_approve`/`admin_timesheet_reject`, `IsGranted('approve_timesheet', 'entry')`.
- [x] T.10 RED functional `tests/Controller/TimesheetTeamControllerTest.php` — approved entry: edit/delete routes return 403 for owner; `is_granted('edit')` false in template context (row-click-to-edit gate closed via existing template wiring, D7).
- [x] T.11 `templates/timesheet/index.html.twig` (the actual template rendered by `TimesheetTeamController::indexAction`, confirmed at apply time — `team_index.html.twig` does not exist) — add approve/reject buttons per row, gated `is_granted('approve_timesheet'/'reject_timesheet', entry)`.
- [x] T.12 Run `phpunit tests/Entity/TimesheetTest.php tests/Voter/TimesheetVoterTest.php tests/Controller/TimesheetTeamControllerTest.php`, full Timesheet test surface (`*Timesheet*`: 872/872 pass), `phpstan analyse` (1 pre-existing unrelated error in `QuotationControllerTest.php`, 0 new), `lint:twig`, `lint:xliff`. PR-T implemented on branch `approval-workflows-expansion-pr-t-timesheet` (base: tracker `approval-workflows-expansion-tracker-timesheet`) — ready for PR to be opened by orchestrator.
