...
- [x] For each accepted direct-to-main slice, perform bounded review of its vertical outcome, tests, allowed surfaces, rollback boundary, and sub-400-line diff before merge; do not begin the next slice until the current slice is merged to `main` and Dokploy deployment is confirmed. <!-- sdd-owner: parent -->
