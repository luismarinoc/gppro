...
- [ ] 4.1 RED: `tests/Entity/LoginAttemptTest.php` — getters/setters; `user` nullable; `outcome` `success`|`failure`; `createdAt` immutable.
- [ ] 4.2 GREEN: `src/Entity/LoginAttempt.php` — `id`, `attemptedUsername` (string 180, not null), `user` (`ManyToOne(User::class)`, nullable, `onDelete: 'SET NULL'`), `ipAddress` (string 45, nullable), `userAgent` (string 255, nullable), `outcome` (string 10, not null), `failureReason` (string 120, nullable), `createdAt` (`datetime_immutable`, not null). Table `gppro_login_attempts`.
- [ ] 4.3 `migrations/VersionYYYYMMDDHHMMSS_login_attempts.php` — `up()` creates `gppro_login_attempts` with FK `user_id` (`SET NULL`), index `IDX_..._CREATED_AT` on `created_at`, composite index `IDX_..._USER` on `(user_id, created_at)`; `down()` drops the table.
- [ ] 4.4 `src/Repository/LoginAttemptRepository.php` + `src/Repository/Query/LoginAttemptQuery.php` — user/date-range/outcome filters, `getPagerfantaForQuery()`, mirrors `UserRepository`/`UserQuery` shape.
- [ ] 4.5 RED: `tests/EventSubscriber/LoginAuditSubscriberTest.php` — `LoginSuccessEvent` persists a `LoginAttempt` with `user` set, `ip`/`userAgent` from `getRequest()`, `outcome=success`. Mirrors `LastLoginSubscriberTest`'s event-construction pattern.
- [ ] 4.6 RED: same file — `LoginFailureEvent` for a known username (`BadCredentialsException`) persists `user` set, `attemptedUsername` matching, `outcome=failure`, `failureReason` = exception's SHORT class name via `(new \ReflectionClass($exception))->getShortName()` — NOT `$exception->getMessage()` (avoids leaking sensitive detail, per design's flagged risk).
- [ ] 4.7 RED: same file — `LoginFailureEvent` for an unknown username (`UserNotFoundException`) persists `user=null`, `attemptedUsername` set, `outcome=failure`, `failureReason` short class name.
- [ ] 4.8 GREEN: `src/EventSubscriber/LoginAuditSubscriber.php` — new subscriber (sibling to `LastLoginSubscriber`, which stays unchanged), subscribes `LoginSuccessEvent` + `LoginFailureEvent`, captures via `getRequest()->getClientIp()` / `User-Agent` header, persists via `LoginAttemptRepository`. Run 4.5-4.7 green.
- [ ] 4.9 RED: `tests/Controller/LoginAuditControllerTest.php` — `ROLE_SUPER_ADMIN` GETs the list, applies user/date/outcome filter, renders scoped results.
- [ ] 4.10 RED: same file — `ROLE_ADMIN` (non-super) is denied 403 (explicit regression guard per locked decision #5).
- [ ] 4.11 RED: same file — records older than any arbitrary age remain queryable without a date filter (no auto-purge).
- [ ] 4.12 GREEN: `src/Controller/LoginAuditController.php` — `#[IsGranted('ROLE_SUPER_ADMIN')]` class-level (native `RoleVoter`, not `RolePermissionManager` — non-reassignable to `ROLE_ADMIN`), `indexAction` mirrors `UserController::indexAction` (query object + DataTable + Pagerfanta + toolbar filter form).
- [ ] 4.13 `templates/login_audit/index.html.twig` — filterable list, mirrors `templates/user/index.html.twig` table+toolbar structure.
- [ ] 4.14 Run `phpunit tests/Entity/LoginAttemptTest.php tests/EventSubscriber/LoginAuditSubscriberTest.php tests/Controller/LoginAuditControllerTest.php`; confirm migration applies and `down()` reverses on the test DB.
- [ ] 4.15 `phpstan analyse`; `lint:twig templates/login_audit/index.html.twig`; `lint:xliff` on any new translation keys; open PR4 (base: main, after PR3 merges).
