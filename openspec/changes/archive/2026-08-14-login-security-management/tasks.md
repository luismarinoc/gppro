...
## Phase 1: Password Policy (PR1, base: main) — COMPLETE

> Archive-time reconciliation note: PR1 (`Merge pull request #122`) genuinely merged to `main` and was independently verified (`sdd/login-security-management/verify-report-pr1`, PASS WITH WARNINGS, 4/4 tasks confirmed complete against source). The dedicated "check off Phase 1" commit was never pushed/merged, so these 4 boxes were left stale on `main` while PR2/PR3/PR4 each got their own checkoff commit. Reconciled here at archive time per verify-report-pr1's completeness table (all 4 tasks independently re-verified against source, not checkbox trust) and per `verify-report-pr4`'s own flagged WARNING #2 recommending this exact cleanup before archive. No functional change.

- [x] 1.1 RED: extend `tests/Entity/UserTest.php` — accepts `Passw0rd` (letter+digit); rejects `Password` (letters-only); rejects `12345678` (digits-only); regression: `Pa1` still rejected on length alone.
- [x] 1.2 GREEN: `src/Entity/User.php:215` — add `#[Assert\Regex(pattern: '/^(?=.*[A-Za-z])(?=.*\d).+$/', message: '...')]` alongside the existing `Assert\Length(min: 8, max: 60, ...)`, same validation groups (`Registration`, `PasswordUpdate`, `UserCreate`, `ResetPassword`, `ChangePassword`). Run 1.1 green.
- [x] 1.3 Confirm all 5 groups still validate identically (single field, single constraint — no per-path duplication needed).
- [x] 1.4 Run `phpunit tests/Entity/UserTest.php`, `phpstan analyse -c tests/phpstan.neon`, `lint:xliff` if a new message key was added; open PR1 (base: main).
...
## Phase 5: Full Regression Sweep (after all 4 PRs merged) — COMPLETE

- [x] 5.1 Run `phpunit tests/Controller/Security/ tests/EventSubscriber/ tests/EventSubscriber/Actions/UserSubscriberTest.php tests/Voter/UserVoterTest.php tests/Entity/UserTest.php tests/Entity/LoginAttemptTest.php tests/Controller/UserControllerTest.php tests/Controller/LoginAuditControllerTest.php` together. Result: **OK (327 tests, 1556 assertions)**, exit 0.
- [x] 5.2 Confirm `tests/EventSubscriber/LastLoginSubscriberTest.php` is untouched and green — zero regression on existing last-login tracking (`LoginAuditSubscriber` is a new sibling, not a replacement). Result: **OK (3 tests, 12 assertions)**.
- [x] 5.3 `phpstan analyse -c tests/phpstan.neon --no-progress` — expect exactly 1 pre-existing, unrelated error (`Controller/QuotationControllerTest.php::decodeJsonResponse()`, per the `expense-approval-by-person` verify-report baseline). Result: confirmed, exactly 1 error, same identifier/location/message as baseline.
- [x] 5.4 `lint:twig` on `templates/security/login.html.twig` + `templates/login_audit/index.html.twig`; `lint:xliff` on all touched translation files. Result: `lint:twig` OK (2/2 valid); `lint:xliff translations/` OK (602/602 valid, includes `messages.en.xlf`/`messages.es.xlf`).
- [x] 5.5 Confirm the migration applies cleanly in sequence on a fresh test DB (`doctrine:migrations:migrate`) and its `down()` reverses cleanly. Result: on `kimai2_test`, `Version20260814130000` was already at head (104/104 executed, 0 new); executed `--down` (2 SQL queries, OK) then `--up` (2 SQL queries, OK) to prove reversibility — status returned to head cleanly both times.
