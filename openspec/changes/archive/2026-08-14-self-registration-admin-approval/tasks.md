...
- [x] 8.6 Open PR2 (base: main, after PR1 merges). RECONCILED at archive time: not opened by the apply agent per explicit orchestrator instruction (do not push/open PR); the orchestrator subsequently pushed and opened PR2 (#131) manually, and it was merged to `main` at commit `8273fec` ("Merge pull request #131 from luismarinoc/self-registration-admin-approval-pr2"), confirmed via `git log --oneline` on `main` at archive time. Checkbox updated by sdd-archive per the Task Completion Gate's exceptional-reconciliation allowance, backed by this git evidence and the archive launch prompt's explicit final-state fact that both PR1 (#126) and PR2 (#131) are merged.
