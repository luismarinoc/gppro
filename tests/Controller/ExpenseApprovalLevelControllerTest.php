 ...
 */
#[Group('integration')]
class ExpenseApprovalLevelControllerTest extends AbstractControllerBaseTestCase
{
    ...
    {
        $this->assertUrlIsSecured('/admin/expense/approval-levels/');
    }

    public function testAdminCannotAccessApprovalLevelManagement(): void
    {
        $this->assertUrlIsSecuredForRole(User::ROLE_ADMIN, '/admin/expense/approval-levels/');
    }
    ...
    public function testNonMonotonicThresholdIsRejected(): void
    {
        ...
        $em = $this->getEntityManager();
        $em->persist((new ExpenseApprovalLevel())->setLevel(2)->setMinAmount(1000000)->setRequiredRole(User::ROLE_ADMIN));
        $em->flush();
        ...
        self::assertNull($em->getRepository(ExpenseApprovalLevel::class)->findOneBy(['level' => 3]));
    }
    ...
    public function testLastRemainingLevelCannotBeDeleted(): void
    {
        ...
        // but the CSRF token id for all delete forms ('expense_approval_level')
        ...
        $level2 = (new ExpenseApprovalLevel())->setLevel(2)->setMinAmount(500000)->setRequiredRole(User::ROLE_ADMIN);
        $em->persist($level2);
        $em->flush();
        ...
        $token = $this->extractToken(
            $client,
            '/admin/expense/approval-levels/',
            'form[action$="/' . $level2Id . '/delete"] input[name=_token]'
        );
        $this->request($client, '/admin/expense/approval-levels/' . $level1Id . '/delete', 'POST', ['_token' => $token]);
    ...
    public function testSuperAdminCanDeleteANonBaseLevel(): void
    {
        ...
        $this->request($client, '/admin/expense/approval-levels/' . $level2Id . '/delete', 'POST', ['_token' => $token]);
        ...
        $em->clear();
        self::assertNull($em->getRepository(ExpenseApprovalLevel::class)->find($level2Id));
    }
}
